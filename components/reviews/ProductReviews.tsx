...
export default function ProductReviews({ productId }: { productId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/products/${encodeURIComponent(productId)}/reviews?sort=${sort}`,
        { cache: "no-store" }
      );
      const result = (await response.json()) as ReviewResponse;
      if (!response.ok) {
        throw new Error(result.error ?? "Review tidak dapat dimuat.");
      }
      setData(result);
    } catch (loadError) {
      setError(
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({ rating, review, images: [] }),
      });
      const result = (await response.json()) as { error?: string };
      if (!response.ok) {
        throw new Error(result.error ?? "Review tidak dapat disimpan.");
      }
      setReview("");
      setRating(5);
      setEditingId(null);
      notify("Review berhasil disimpan.");
      await load();
    } catch (saveError) {
      setError(
        saveError instanceof Error
          ? saveError.message
          : "Review tidak dapat disimpan."
      );
    } finally {
      setSaving(false);
    }
  ...
  async function remove(id: string) {
    ...
    if (response.ok) {
      notify("Review berhasil dihapus.");
      await load();
    } else {
      setError("Review tidak dapat dihapus.");
    }
    setSaving(false);
  }
    ...
    <section className={styles.section} id="reviews">
      ...
      {!showForm && !ownReview && (
        <p className={styles.eligibility}>
          Hanya pembeli terverifikasi yang dapat memberi review.
        </p>
      )}
