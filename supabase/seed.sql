...
insert into public.products (slug, name, category_id, price, compare_price, weight, status, shopee_url)
select 'hoodie-catchus-inverted-cross-oversize-330gsm',
       'Hoodie Catchus -Inverted cross- Oversize 330GSM',
       cat.id, 247500, 400000, 0.90, 'active', 'https://s.shopee.co.id/4foSmdo80Z'
from cat
on conflict (slug) do nothing;
...
select 'beanie-hat-catchus-inverted-cross-knit-cotton-rajut',
       ...
       cat.id, 66000, 120000, 0.30, 'active', 'https://s.shopee.co.id/8fKbYII0tm'
from cat
on conflict (slug) do nothing;
...
select 'catchus-club-direction-of-war-boxy-oversize-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/20nhbmaJTw'
from cat
on conflict (slug) do nothing;
...
select 'Catchus-club-army-series-white-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/12dEt6XDg'
from cat
on conflict (slug) do nothing;
...
select 'Catchus-club-the-great-progress-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/7plUYYdm2a'
from cat
on conflict (slug) do nothing;
...
select 'Catchus-club-the-flower-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/2LQY0TxQH5'
from cat
on conflict (slug) do nothing;
...
select 'catchus-club-biggest-boxy-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/5fgzyd2PQm'
from cat
on conflict (slug) do nothing;
...
select 'catchus-club-never-seen-by-anyone-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/6KwglsTqx7'
from cat
on conflict (slug) do nothing;
...
select 'catchus-club-human-of-their-time-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/6Kwgmqj92S'
from cat
on conflict (slug) do nothing;
...
select 'catchus-club-stop-killing-black-people-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/BM3Ri1uXB'
from cat
on conflict (slug) do nothing;
...
select 'catchus-club-army-series-black-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/60JqNLtyvM'
from cat
on conflict (slug) do nothing;
...
select 'catchus-club-c.u.c-ak-47-tshirt',
       ...
       cat.id, 110000, 200000, 0.30, 'active', 'https://s.shopee.co.id/7V8eA89jDs'
from cat
on conflict (slug) do nothing;
...
select 'catchus-club-~c.u.c-chrome~-boxy-tshirt',
       ...
       cat.id, 135000, 200000, 0.30, 'active', 'https://shopee.co.id/Catchus-Club-~C.U.C-CHROME~-Boxy-Tshirt-i.742978637.56912974905?extraParams=%7B%22display_model_id%22%3A386125757796%2C%22model_selection_logic%22%3A3%7D'
from cat
on conflict (slug) do nothing;
